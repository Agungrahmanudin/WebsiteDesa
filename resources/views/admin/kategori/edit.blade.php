...
@section('title', 'Edit Kategori - Admin Desa Cimeong')
@section('header-title', 'Edit Kategori Berita')
...
<div class="card bg-white dark:bg-zink-700 border border-slate-200 dark:border-zink-500 rounded-xl p-6 shadow-sm w-full">
    <div class="flex items-center justify-between pb-4 mb-6 border-b border-slate-100 dark:border-zink-600">
        <div>
            <h5 class="text-base font-semibold text-slate-800 dark:text-zink-50 mb-0.5">Edit Kategori Berita</h5>
            <p class="text-xs text-slate-500 dark:text-zink-300">Perbarui data kategori artikel dan berita desa</p>
        </div>
    ...
    <form action="{{ route('admin.kategori.update', $kategori->id) }}" method="POST">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div class="md:col-span-2">
                ...
                <input type="text" name="nama_kategori" class="w-full text-xs px-3.5 py-2.5 border rounded-lg border-slate-200 dark:border-zink-500 dark:bg-zink-700 focus:outline-none focus:border-custom-500" value="{{ old('nama_kategori', $kategori->nama_kategori) }}" required>
            </div>
            <div class="md:col-span-2">
                <label class="block text-xs font-semibold text-slate-600 dark:text-zink-200 mb-1.5">Deskripsi</label>
                <textarea name="deskripsi" rows="4" class="w-full text-xs px-3.5 py-2.5 border rounded-lg border-slate-200 dark:border-zink-500 dark:bg-zink-700 focus:outline-none focus:border-custom-500">{{ old('deskripsi', $kategori->deskripsi) }}</textarea>
            </div>
        </div>
